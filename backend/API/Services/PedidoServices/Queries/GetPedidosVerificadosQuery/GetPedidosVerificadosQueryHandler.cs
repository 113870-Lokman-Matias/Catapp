...
namespace API.Services.PedidoServices.Queries.GetPedidosVerificadosQuery
{
  public class GetPedidosVerificadosQueryHandler : IRequestHandler<GetPedidosVerificadosQuery, ListaPedidosDto>
  {
    private readonly CatalogoContext _context;
    private readonly IMapper _mapper;

    public GetPedidosVerificadosQueryHandler(CatalogoContext context, IMapper mapper)
    {
      _context = context;
      _mapper = mapper;
    }

    public async Task<ListaPedidosDto> Handle(GetPedidosVerificadosQuery request, CancellationToken cancellationToken)
    {
      try
      {
        var pedidos = await _context.Pedidos
        .Where(x => x.Verificado == true)
